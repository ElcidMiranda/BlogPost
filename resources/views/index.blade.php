    ...
    <div class="w-full flex flex-row justify-between flex-wrap py-4 text-xl text-white bg-[#1b1b1b] shadow border ">
        ...
        <div class="div">
            @if (Route::has('login'))
                <div class="">
                    @auth
                        <a href="{{ route('dashboard') }}" class="text-white font-semibold hover:text-gray-900 dark:text-gray-400
    ...
    <header class ="container w-full mx-auto">
        ...
        <nav class="w-full py-4 border-t border-b bg-gray-100">
            <div class="w-full container mx-auto flex flex-col sm:flex-row items-center justify-center text-sm font-bold uppercase mt-0 px-6 py-2">
                ...
                <a href="{{ route('index','tag='.$category->id) }}" class="hover:bg-gray-400 rounded py-2 px-4 mx-2">{{ $category->categoryName }}</a>
                @endforeach
            </div>
            <form>
